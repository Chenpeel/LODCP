...
\usepackage[backend=biber,style=gb7714-2015,gbnamefmt=lowercase,gbpub=false,url=true]{biblatex}
...
具体而言，本研究结合Fast-SCNN深度学习模型与传统图像处理技术，采用多项式曲线拟合方法实现车道线精确识别与建模，显著提高了系统在复杂光照和恶劣天气条件下的鲁棒性。针对嵌入式平台的资源限制，采用轻量级YOLOv5n模型进行障碍物检测，并运用知识蒸馏、模型剪枝和量化等技术优化算法，以YOLOv5s作为教师模型提升检测性能，最终实现了算法的高效嵌入式部署。


在跟踪算法方面，本研究基于DeepSORT框架进行优化，引入IoU度量机制与类别感知匹配策略，显著提高了多目标跟踪的准确性和鲁棒性。此外，通过构建综合风险评估模型，系统能够实时分析潜在碰撞风险并发出预警。该模型综合考虑时空风险因子、轨迹交叉程度和环境条件等因素，为驾驶安全决策提供了可靠支持。实验结果表明，所提方法在障碍物识别精度、跟踪稳定性和碰撞预警时效性等方面均表现优异，特别适合计算资源受限的嵌入式环境应用。
...
\begin{parlist}
    ...
    \item 目标检测种类不足：目标检测模型的训练类别较少，这对后期碰撞预测的分级、分类别碰撞预测有一定影响，分类较少可能导致碰撞预测概率失误。
...
\begin{parlist}
    \item 丰富数据集中的类别，通过多数据集融合，训练更多类别的目标检测，丰富障碍物种类，从而对不同障碍物的碰撞风险因子改进，达到更优的碰撞预测效果，降低在一般场景下的误报率。
