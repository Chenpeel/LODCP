...
% \usepackage[
%   backend=biber,
%   style=numeric,     % 确保使用数字样式
%   sorting=none,      % 按引用顺序排序
%   citestyle=numeric, % 明确设置引用样式为数字
%   bibstyle=numeric   % 明确设置参考文献样式为数字
% ]{biblatex}
...
\setlength{\jot}{0.5\baselineskip} % 设置多行公式的行距为1.5倍
\AtBeginDocument{%
  \normalsize
  \everydisplay{\abovedisplayskip=12pt\belowdisplayskip=12pt}%
}

% 设置字体
\setmainfont{Times New Roman}
\setCJKmainfont[BoldFont={Heiti SC}]{Songti SC} % Mac系统宋体和黑体
...
\setlist[parlist,2]{
  label=(\alph*),
  leftmargin=4.5em,  % 增加左边距
  labelindent=2em,   % 设置标签缩进
  labelsep=0.5em,
  stdformat,
...
\captionsetup[figure]{font=small, labelsep=space, textformat=simple}
\captionsetup[table]{font=small, labelsep=space, textformat=simple}
...
\pagestyle{fancy}
\fancyhf{} % 清除所有页眉页脚
\fancyhead[C]{\fontsize{14pt}{16.8pt}\selectfont\songti 安徽理工大学本科毕业设计说明书} % 四号宋体
...
\vspace*{0.3cm}
{\zihao{-0} \textbf{本科毕业设计说明书（论文）}}
\vspace{1.5cm}

\begin{minipage}{15cm}
% 中文题目：宋体，三号字，加粗
\zihao{3}\songti\bf
\noindent \textbf{中文题目：}\underline{\centering{\textbf{基于深度学习的车道障碍检测与碰撞预测方法研究}}}

\vspace{0.8cm}

% 英文题目：Times New Roman，三号字，加粗

\noindent \textbf{英文题目：}
\zihao{3}\rmfamily\bfseries
\underline{\centering{\textbf{Research On Deep Learning-Based Lane Obstacle}}}
\underline{\centering{\textbf{Detection And Collision Prediction Methodology}}}

\vspace{1.5cm}

% 个人信息：宋体，三号，加粗
\zihao{3}\songti\bf
\centering\textbf{学\hspace{2em}院：}\underline{\makebox[14em][c]{\textbf{计算机科学与工程学院}}}

\vspace{0.3cm}
\centering\textbf{专\hspace{2em}业：}\underline{\makebox[14em][c]{\textbf{计算机科学与技术}}}

\vspace{0.3cm}
\centering\textbf{班\hspace{2em}级：}\underline{\makebox[14em][c]{\textbf{21计科4班}}}

\vspace{0.3cm}
\centering\textbf{学\hspace{2em}号：}\underline{\makebox[14em][c]{\textbf{2021304369}}}

\vspace{0.3cm}
\centering\textbf{姓\hspace{2em}名：}\underline{\makebox[14em][c]{\textbf{陈亚辉}}}

\vspace{0.3cm}
\centering \textbf{指导老师：}\underline{\makebox[14em][c]{\textbf{郑苹讲师}}}
\vspace{0.3cm}
...
近年来，中国汽车产业快速发展，智能驾驶技术持续创新，但交通事故发生率仍居高不下。传统道路障碍检测方法存在精度不足、计算复杂度高和人工依赖性强等局限性。为此，本研究提出了一种基于深度学习的车道障碍检测与碰撞预测系统。通过融合传统图像处理与深度学习技术，设计了多模块并行处理架构，有效提升了复杂场景下的障碍物识别与风险预警能力。


具体而言，本研究结合Fast-SCNN深度学习模型与传统图像处理技术，采用多项式曲线拟合方法实现车道线精确识别与建模，显著提高了系统在复杂光照和恶劣天气条件下的鲁棒性。针对嵌入式平台的资源限制，采用轻量级YOLOv5n模型进行障碍物检测，并运用知识蒸馏、模型剪枝和量化等技术优化算法，以YOLOv5s作为教师模型提升检测性能，最终实现了算法的高效嵌入式部署。在跟踪算法方面，本研究基于DeepSORT框架进行优化，引入IoU度量机制与类别感知匹配策略，显著提高了多目标跟踪的准确性和鲁棒性。此外，通过构建综合风险评估模型，系统能够实时分析潜在碰撞风险并发出预警。该模型综合考虑时空风险因子、轨迹交叉程度和环境条件等因素，为驾驶安全决策提供了可靠支持。实验结果表明，所提方法在障碍物识别精度、跟踪稳定性和碰撞预警时效性等方面均表现优异，特别适合计算资源受限的嵌入式环境应用。
...
In recent years, China's automotive industry has developed rapidly, and intelligent driving technologies have continued to advance, yet traffic accident rates remain persistently high. Traditional road obstacle detection methods suffer from limitations such as insufficient accuracy, high computational complexity, and strong reliance on manual intervention. To address these challenges, this study proposes a deep learning-based lane obstacle detection and collision prediction system. By integrating traditional image processing with deep learning techniques, a multi-module parallel processing architecture is designed, significantly improving obstacle recognition and risk warning capabilities in complex scenarios.

Specifically, this research combines the Fast-SCNN deep learning model with traditional image processing techniques, employing polynomial curve fitting for precise lane line identification and modeling. This approach notably enhances system robustness under challenging lighting and adverse weather conditions. Considering the resource constraints of embedded platforms, a lightweight YOLOv5n model is adopted for obstacle detection, with algorithm optimization techniques including knowledge distillation, model pruning, and quantization. Using YOLOv5s as a teacher model to enhance detection performance, the study achieves efficient embedded deployment. For tracking algorithms, this work optimizes the DeepSORT framework by introducing an IoU metric mechanism and a category-aware matching strategy, substantially improving multi-object tracking accuracy and robustness. Furthermore, by constructing a comprehensive risk assessment model, the system can analyze potential collision risks in real time and issue warnings. This model incorporates spatiotemporal risk factors, trajectory intersection severity, and environmental conditions to provide reliable support for driving safety decisions. Experimental results demonstrate that the proposed method excels in obstacle recognition precision, tracking stability, and collision warning timeliness, making it particularly suitable for resource-constrained embedded environments.
...
\end{center}
\caption{\textbf{各向异性扩散滤波图}}
\label{fig:ani_diffusion}
\end{figure}

\end{parlist}

在实际车载系统开发中，需根据传感器特性和环境条件选择适配的滤波策略，CMOS相机受限于传感器特性易产生椒盐噪声，采用中值滤波预处理；而在低光照条件下，高斯滤波结合亮度自适应阈值可提升夜间视觉效果；对于需要实时处理的场景，可采用积分图技术加速均值滤波，将复杂度降至$O(1)$；对于道路常见的光照不均匀场景，采用各向异性扩散滤波获取更优的特征。
...
\begin{halfparlist}
    ...
    \item 基于二阶导数的检测算法，采用拉普拉斯等二阶差分运算定位边缘区域，如LoG（Laplacian of Gaussian）\cite{kong2013generalized}；

    \item 基于Canny准则的多阶段算法，通过系统化处理实现亚像素级边缘提取；

    \item 基于多尺度分析的高级方法，如DoG（Difference of Guassian）\cite{chen2021joint}和小波变换，能够捕获不同尺度的边缘特征；

    \item 基于深度学习的端到端方法，如HED（Holistically-Nested Edge Detection），RCF（Richer Convolutional Features for Edge Detection）等\cite{xie2015holistically,liu2017richer}，直接从数据中学习边缘特征表示。
\end{halfparlist}
...
霍夫变换（Hough Transform）\cite{illingworth1988survey}是用于检测参数化几何形状（如直线、圆、椭圆等）的经典特征提取技术，其核心是将笛卡尔坐标系中的点映射到参数空间，通过累加统计找出最可能的几何形状。
...
现代目标检测技术已由早期的传统特征提取方法，如HOG（Histogram of Oriented Gradients）、SIFT（Scale-Invariant Feature Transform）等\cite{dalal2005histograms,lowe2004distinctive}。发展为基于深度学习的端到端解决方案，大幅提升了检测精度和鲁棒性。
...
\caption{\textbf{YOLOv5模型在COCO数据集测试结果 \cite{ultralytics2023}}}
...
\begin{table}[H]
\centering
\caption{\textbf{YOLOv5模型系列参数对比 \cite{ultralytics2023}}}
...
\begin{figure}[H]
    ...
    \begin{center}
    (a) 目标检测（昼） \hspace{3cm} (b) 目标检测（夜）
    \end{center}
    \caption{\textbf{复杂路况检测效果}}
    \label{fig:detection}
\end{figure}
...
\begin{halfparlist}
    \item 边界框宽高比：$r = w/h$，此比值能有效表征目标的形态特征
    \item 尺寸一致性：$s_{cons} = \min(w_1/w_2, w_2/w_1) \cdot \min(h_1/h_2, h_2/h_1)$，用于评估前后帧目标大小变化的合理性
    \item 运动一致性：通过速度向量余弦相似度计算，反映目标运动方向的连续性
\end{halfparlist}

这些精心选取的特征组合构成了一个既高效又实用的轻量级表观描述符，能够在计算资源有限的条件下有效区分不同目标。

\item IoU作为主要相似度度量

本文主要依赖IoU（交并比，Intersection over Union）作为目标关联度量标准。这一选择源于IoU能够有效地表征两个边界框之间的空间重叠关系，为目标跟踪提供稳定可靠的度量基准。IoU的计算公式如下：

\begin{equation}
IoU(A, B) = \frac{\text{Area}(A \cap B)}{\text{Area}(A \cup B)}
\end{equation}

其中，$A$表示预测框，$B$表示检测框，$A \cap B$表示两个框的交集区域面积，$A \cup B$表示两个框的并集区域面积。IoU值范围为$[0,1]$，值越大表示两个框的重叠程度越高，相似度越大。

与传统的马氏距离等纯距离度量相比，IoU具有以下显著优势：

\begin{halfparlist}
\item 几何解释明确：直观地表达了两个边界框的重叠程度，便于理解和算法调整
\item 尺度不变性：不受目标大小变化的影响，对近处大目标和远处小目标均能公平对待
\item 形状敏感：综合考虑了边界框的形状和方向信息，而非仅仅关注中心点距离
\item 计算效率高：实现简单，计算开销较小，适合嵌入式平台部署
\end{halfparlist}
...
其中，$i$代表跟踪轨迹中的目标，$j$代表当前帧检测到的目标。在数据关联过程中，当$d_{IoU}(i,j) \leq t_{IoU}$时（其中$t_{IoU}$为预设阈值），检测框$j$与跟踪目标$i$被认为具有关联可能性，进入后续匹配流程。

此外，针对复杂场景中的目标跟踪挑战，我们对基本IoU度量进行了改进，综合考虑了目标的运动一致性和外观特征相似度，从而进一步提高了跟踪算法的鲁棒性和准确性。

\item 类别感知匹配

本文在关联阶段引入了目标类别信息，实现了类别约束匹配机制，即仅允许同类目标之间的匹配。这一策略显著提升了多类别目标跟踪的准确性，有效避免了不同类别目标之间的错误关联，特别是在目标密集交叉的复杂场景中表现出色。


这种经过精心设计的特征度量方案实现了在资源受限的嵌入式设备上的实时目标跟踪，同时保持了足够的精度用于驾驶决策支持。跟踪效果如图\ref{fig:track_result}所示。
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\linewidth}
        ...
        （b）第300帧跟踪结果
    \end{minipage}
    \caption{\textbf{障碍物跟踪效果展示}}
    \label{fig:track_result}
\end{figure}

通过以上系统性优化，本文提出的障碍物跟踪算法能够在资源受限的嵌入式设备上高效运行，同时保持车载场景所需的跟踪精度和稳定性。系统在模拟环境和真实道路测试中均展现出稳定的跟踪性能和鲁棒性，为后续的碰撞风险评估提供了可靠的数据基础，有力支持了安全驾驶辅助功能的实现。
\end{parlist}
...
\begin{parlist}
    ...
    \item 高效多目标跟踪：改进DeepSORT算法框架，设计了适合车载环境的目标跟踪方案。通过优化卡尔曼滤波器参数和基于IoU的关联度量，在保持高精度跟踪的同时，将计算复杂度降低了50\%以上，使系统能够在树莓派、等资源受限平台上稳定运行。
...
本研究虽在车道障碍检测与碰撞预测领域取得一定进展，但仍存在局限性，这些局限也指明了未来研究方向：

\begin{parlist}
    \item 环境适应性问题与解决路径：当前系统在极端天气（大雪、浓雾）和低光照环境下表现不佳，尤其是雨雪导致的路面反光和模糊视野条件下，车道线检测准确率显著下降。未来研究将整合摄像头、毫米波雷达和超声波传感器优势，构建多模态感知系统，提高全天候全场景感知能力，特别是在视觉受限条件下，利用雷达提供可靠的距离和速度信息。

    \item 目标检测类别扩展：目前训练的检测类别较少，限制了碰撞预测的分级精度。未来将通过多数据集融合，训练更丰富类别的目标检测模型，优化不同障碍物的碰撞风险评估因子，提高预测准确性，降低一般场景的误报率。

    \item 目标遮挡与高级跟踪技术：在严重遮挡场景下，特别是车流密集的城市道路环境中，目标轨迹预测存在不稳定性，部分遮挡导致的ID切换问题影响碰撞风险评估连续性。未来将引入基于注意力机制的时空序列模型（如Transformer架构），提升系统对交通参与者意图和行为的预测能力，使预警系统从碰撞预测向行为理解与风险推理方向发展。

    \item 计算资源优化策略：为适应嵌入式平台，当前模型参数大幅精简，牺牲部分检测精度，导致在复杂场景可能漏检小尺寸或远距离目标。未来将设计边缘计算和云服务协同的分层系统架构，在确保核心安全功能本地实时响应的同时，利用云端资源进行模型更新、场景分析和历史数据挖掘，提高系统智能化水平。

    \item 传感器融合与车路协同：当前系统主要依赖单一摄像头数据，缺乏多传感器融合能力，限制了感知深度和全天候工作能力。未来将研究基于5G/V2X通信的车路协同感知技术，利用路侧基础设施信息扩展单车感知边界，形成网络化的协同感知与风险预警系统，尤其针对视线受限的交叉路口等高风险区域。

    \item 动态建模与自适应学习：采用的线性运动模型难以准确预测复杂交通参与者的突变行为，对急转弯、突然减速等非线性运动模式预测精度有限。未来将探索在线学习和增量更新机制，使系统能从实际驾驶数据中持续优化，自动适应不同驾驶习惯和道路环境，形成个性化安全辅助策略。

    \item 人机交互优化：未来将深入研究驾驶员认知模式和注意力分配特性，优化预警信息的呈现方式和触发时机，提高驾驶员对系统预警的接受度和反应效率，减少误报和漏报导致的用户体验问题。
\end{parlist}

本研究为基于深度学习的车道障碍检测与碰撞预测方法提供了实用性解决方案，为提升道路交通安全贡献了新的技术手段。随着计算硬件性能提升、深度学习算法进步以及多传感器融合技术发展，未来的辅助驾驶感知系统将更加智能、可靠，为实现"零事故"交通愿景提供坚实技术支撑。
