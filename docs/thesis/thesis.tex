...
% 设置图表标题格式
\captionsetup[figure]{font=small, labelsep=space} % 五号约为10.5pt
\captionsetup[table]{font=small, labelsep=space} % 五号约为10.5pt
...
% 目录格式
\renewcommand{\cftsecfont}{\bfseries\songti} % 一级标题黑体
\renewcommand{\cftsecpagefont}{\bfseries\songti} % 页码也用黑体
\renewcommand{\cftsubsecfont}{\songti} % 二级标题宋体
\renewcommand{\cftsubsecpagefont}{\songti} % 页码也用宋体
...
从图\ref{fig:processing_time}可以明显观察到，蒸馏后的YOLOv5nu模型相较于原始教师模型YOLOv5su，推理速度提高至其171\%（使用Kaggle 单GPU T4 15G进行速度测试）。这一速度提升对于自动驾驶系统的实时决策至关重要，使系统能够更快地响应环境变化。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/chapter4/ptc.png}
    \caption{Detailed comparison of processing time across different stages for each model.}
    \label{fig:processing_time}
\end{figure}

同时，在对处理要求较严格的智能驾驶方向，FPS十分重要，如图\ref{fig:fps_comparison}，在保留了YOLOv5nu的处理速度的同时，较好地保留了YOLOv5su的精度。


\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/chapter4/fps.png}
    \caption{Frame rate (FPS) comparison between different models.}
